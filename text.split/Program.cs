...
namespace text.split
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (Word wo in words.OrderByDescending(o => o.count).ToList())
            {
                float percent = (float)Math.Round(((float)wo.count / (float)strList.Count) * 100,2);
                string txt = String.Format(@"""{0}"": {1} {2} {3}%", wo.word, wo.count, (wo.count != 1)?"occurences":"occurence", percent) + Environment.NewLine;
                Console.Write(txt);
